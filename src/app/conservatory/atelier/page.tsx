      ...
      <div className="mt-10 grid gap-4">
        {roots.map((category) => (
          <div key={category.id} className="rounded-2xl border border-white/10 p-5">
            <Link href={`/conservatory/atelier/${category.id}`} className="block hover:text-aqua">
            ...
            {category.children.length > 0 && (
              <ul className="mt-4 space-y-2 border-t border-white/8 pt-4">
                {category.children.map((child) => (
                  <li key={child.id}>
                    <Link href={`/conservatory/atelier/${child.id}`} className="flex items-center justify-between text-sm text-sand/75 hover:text-white">
                      <span>{child.name}</span>
                      <span className="text-[11px] tracking-[0.14em] text-sand/45 uppercase">
