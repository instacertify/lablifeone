...
export function HouseForm({ settings }: { settings: Setting }) {
  ...
  return (
    <form onSubmit={onSubmit} className="mt-10 grid gap-4 sm:grid-cols-2">
      <div className="rounded-2xl border border-ink/10 p-5 sm:col-span-2">
        <p className="text-[11px] tracking-[0.18em] text-ink/55 uppercase">House mark</p>
      ...
      {(
        ...
        <label key={name} className="text-xs text-ink/55">
          {label}
      ...
      {status && <p className="self-center text-sm text-ink/70">{status}</p>}
    </form>
  );
}
